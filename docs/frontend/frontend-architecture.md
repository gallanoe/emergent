...
- **Swarm-connection subsystem** — `appState` still carries `agentConnections` state, the `connect/disconnect/refreshConnections` methods, and a `swarm:topology-changed` listener, all left over from the removed swarm view. Nothing in `src/` reads them (the sidebar's "swarm" button now just shows the overview). Note the internal selection ids are still named `selectedSwarm`/`selectedSwarmId` — a _workspace_ is the "swarm" in old naming.
